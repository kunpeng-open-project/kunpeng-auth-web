<template>
  <el-card shadow="always">
    ...
    <el-table v-if="tableShow" ref="tableRef" v-loading="loading" :data="tableList" :style="tableHeaderStyles" :size="tableSize" :height="tableHeight" fit :row-key="rowKey" :default-expand-all="tableIsFold" :tree-props="{ children: 'children' }" @sort-change="sortChangeOrderBy" @selection-change="multiSelect">
      ...
      <!--      :render-header="renderHeader" 使用监听器代替-->
      <el-table-column v-for="(column, index) in tableColumn" :key="column.prop" :prop="column.prop" :label="column.label" :min-width="column.width" :align="index === 0 ? '' : 'center'" :sortable="column.sort === undefined ? false : 'custom'" :sort-orders="['ascending', 'descending']" show-overflow-tooltip>
...
import { treeToList, treeToListNumber } from "@/utils/kp/tool/tree"
