<template>
  <div class="main">
    <KPTableQuery :event-bus="eventBus" :query-params="queryParams">
      <KPSelectApi v-model="queryParams.projectId" label="所属项目" api="/project/select" :api-params="{ manageType: 1 }" :span="6" @change="kpSelectChange(eventBus, queryParams)" />
      <KPInputText v-model="queryParams.dictName" label="字典名称" :span="6" />
      <KPInputText v-model="queryParams.dictType" label="字典类型" :span="6" />
    ...
    <KPTable :event-bus="eventBus" :query-params="queryParams" :table-key="basic.tableKey" :table-column="tableColumn" :list-api="basic.listApi" :del-api="basic.delApi" :add-button-auth="basic.addButtonAuth" :update-button-auth="basic.updateButtonAuth" :del-button-auth="basic.delButtonAuth" :details-button-auth="basic.detailsButtonAuth" details-button-row update-button-row del-button-row checkbox>
      <template #status="{ row }">
        <el-switch v-if="hasAuth('auth:dict:type:do:status')" v-model="row.status" inline-prompt :active-value="1" active-text="正常" :inactive-value="0" inactive-text="停用" @click="handleSwitchStatus(row)" />
        <el-tag v-if="!hasAuth('auth:dict:type:do:status') && row.status == 1" type="success" round effect="dark">正常</el-tag>
        <el-tag v-if="!hasAuth('auth:dict:type:do:status') && row.status == 0" type="danger" round effect="dark">停用</el-tag>
      </template>

      <template #actions="{ row }">
        <el-button type="primary" size="small" title="设置数据" link @click="toPage('query', { dictTypeId: row.dictTypeId })">
          <IconifyIconOnline icon="fa-solid:warehouse" />
          &nbsp;设置数据
        </el-button>
      </template>
    </KPTable>
...
let detailsColumn: DetailsColumn[] = [
  ...
  { prop: "projectIds", label: "项目id集合", span: 2, isList: ", " },
  { prop: "projectNames", label: "项目名称集合", span: 2, isList: ", " },
