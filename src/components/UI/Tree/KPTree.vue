...
watch(() => [props.expandedKeys, props.treeValue, props.checkedKeys], async ([expandedKeys, treeValue, checkedKeys]) => {
	...
	if (checkedKeys) {
		...
		let selectAllInterval = setInterval(() => {
			...
			if (allNodes.length === checkedNodes.length)
				isSelectAllChecked.value = true;
			if (num >= 15) clearInterval(selectAllInterval);
		}, 40)
	}
},{ deep: true, immediate: true }) // 加上 immediate 更稳妥);
