<template>
  <el-table v-loading="loading" :data="tableList.list" :style="tableHeaderStyles" :size="tableSize" :height="isControlHeight ? tableHeight : undefined" @sort-change="sortChangeOrderBy" @selection-change="multiSelect" fit>
    ...
    <el-table-column v-for="column in tableColumn" :key="column.prop" :prop="column.prop" :label="column.label" :width="column.width" align="center" :sortable="column.sort === undefined ? false : 'custom'" :sort-orders="['ascending', 'descending']" :show-overflow-tooltip="true">
...
// 接收父组件的值 变成普通数据
let { listApi, queryParams, kpTableQueryHeight } = props
//table列表数据
