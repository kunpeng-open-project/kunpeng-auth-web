<template>
	<div class="main">
		...
		<KPTable :event-bus="eventBus" :list-api="basic.listApi" :table-column="tableColumn" checkbox :add-button="basic.addButtonAuth" :update-button="basic.updateButtonAuth" :del-button="basic.delButtonAuth" :details-button-row="basic.detailsButtonAuth" update-button-row del-button-row :table-key="basic.tableKey" :del-api="basic.delApi" action-width="350px">
			...
			<template #manage="{ row }">
				<el-tag v-if="row.manage === 0" type="warning">不管理</el-tag>
				<el-tag v-if="row.manage === 1" type="success">管理</el-tag>
			</template>
			...
			<template #actions="{ row }">
				<el-button v-if="hasAuth('auth:project:menu:setting:install')" type="primary" size="small" title="设置权限" link @click="openDrawer(row)">
					<IconifyIconOnline icon="ep:menu"/>&nbsp;设置权限
				</el-button>
				
				<el-button type="primary" size="small" :title="'打开' + row.projectName" link>
					<IconifyIconOnline icon="ri:openai-fill"/>&nbsp
		...
		<KPDialogDetails :event-bus="eventBus" v-model="detailsDialogVisible" :title="basic.title + '详情'" :details-api="basic.detailsApi" :table-key="basic.tableKey" :details-column="detailsColumn" label-width="160px"/>
		...
		<KPDrawer v-model="drawerVisible" title="设置权限" :event-bus="eventBus" @drawerSave="drawerSave">
			<el-form :model="editForm" label-width="80px">
				...
				<KPGroupForm label="接口">
					<KPTree ref="treeMenuRef" occupyHeight="350px" placeholder="请输入菜单进行搜索" :width="90" :tree-value="projectMenuSelectValue" :expanded-keys="defaultProjectMenuSelectValue" :checked-keys="defaultCheckedMenuSelectValue" checkbox></KPTree>
				</KPGroupForm>
			</el-form>
		</KPDrawer>
	</div>
...
let tableColumn: TableColumn[] = [
	...
	{ prop: 'manage', label: '管理状态', sort: true },
