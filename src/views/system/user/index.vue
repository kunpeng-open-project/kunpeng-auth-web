<template>
	<div class="main">
		<el-row :gutter="10">
			...
			<el-col :span="21">
				...
				<KPTable ref="tableTreeRef" :query-params="queryParams" :event-bus="eventBus" :list-api="basic.listApi" :table-column="tableColumn" checkbox :add-button="basic.addButtonAuth" :update-button="basic.updateButtonAuth" :del-button="basic.delButtonAuth" :details-button-row="basic.detailsButtonAuth" :table-key="basic.tableKey" :del-api="basic.delApi" action-width="70px">
					...
					<template #status="{ row }">
						<el-switch v-if="row.status == 1 || row.status == 2" v-model="row.status" inline-prompt :active-value="1" active-text="正常" :inactive-value="2" inactive-text="禁用" @click="handleSwitchStatus(row)"/>
						<el-tag v-if="row.status == 3" type="warning">锁定</el-tag>
						<el-tag v-if="row.status == 4" type="danger">注销</el-tag>
					</template>
				</KPTable>
			</el-col>
		</el-row>
		...
		<KPDialogFormEdit :event-bus="eventBus" v-model="dialogVisible" :rules="rules" :title="basic.title" :edit-params="editForm" :date-structure="EditData" :save-api="basic.saveApi" :details-api="basic.detailsApi" :table-key="basic.tableKey" :update-api="basic.updateApi" label-width="100px">
			...
			<KPGroupForm label="归属部门">
				<template v-for="(userDept, index) in editForm.userDepts">
					<div :style="index==0? 'margin-top: 0px' : 'margin-top: 46px'"></div>
...
let detailsColumn: DetailsColumn[] = [
	...
	{ prop: 'idCard', label: '身份证', span: 3 },
	{ prop: 'loginDate', label: '最后登陆时间' },
	{ prop: 'loginIp', label: '最后登陆IP' },
	{ prop: 'lockDate', label: '锁定时间', span: 2 },
	{ prop: 'deptIds', label: '部门Id集合', span: 3, isList: true },
	{ prop: 'deptNames', label: '部门名称集合', span: 3, isList: true },
	{ prop: 'roleIds', label: '角色Id集合', span: 3, isList: true },
	{ prop: 'roleNames', label: '角色名称集合', span: 3, isList: true },
	{ prop: 'postIds', label: '岗位Id集合', span: 3, isList: true },
	{ prop: 'postNames', label: '岗位名称集合', span: 3, isList: true },
	{ prop: 'projectIds', label: '可操作项目id', span: 3, isList: true },
	{ prop: 'projectNames', label: '可操作项目名称', span: 3, isList: true },
...
class EditData {
	...
	userDepts: Array<{ deptId: string, principal: number }> = [
		{ deptId: null, principal: 0 }
	];
};
//编辑表单数据
