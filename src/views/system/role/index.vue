		...
		<KPTable ref="tableTreeRef" :query-params="queryParams" :event-bus="eventBus" :list-api="basic.listApi" :table-column="tableColumn" checkbox :add-button="basic.addButtonAuth" :update-button="basic.updateButtonAuth" :del-button="basic.delButtonAuth" :details-button-row="basic.detailsButtonAuth" update-button-row del-button-row :table-key="basic.tableKey" :del-api="basic.delApi" action-width="350px">
			...
			<template #actions="{ row }">
				<el-button v-if="hasAuth('auth:role:menu:setting:install')" type="primary" size="small" title="菜单权限" link @click="openDrawer(OperateEnum.setMenuAuthority, row)">
					<IconifyIconOnline icon="ep:menu"/>&nbsp;菜单权限
				</el-button>
				...
				<el-button v-if="hasAuth('auth:role:permission:setting:install')" type="primary" size="small" title="菜单权限" link @click="openDrawer(OperateEnum.setDataAuthority, row)">
					<IconifyIconOnline icon="ri:database-2-fill"/>&nbsp;数据权限
				</el-button>
			</template>
		</KPTable>
			...
			<el-form :model="editForm" label-width="80px">
				...
				<template v-if="drawerTitle === '设置数据权限'">
					...
					<KPGroupForm label="部门权限" v-if="editForm.permissionType===5">
						<KPTree ref="treeDeptRef" occupyHeight="350px" placeholder="请输入部门进行搜索" :width="90" :tree-value="deptSelectValue" :expanded-keys="defaultdeptSelectValue" :checked-keys="defaultCheckedDeptSelectValue" checkbox></KPTree>
					</KPGroupForm>
				</template>
...
let detailsColumn: DetailsColumn[] = [
	...
	{ prop: 'projectNameList', label: '所属项目', span: 2, isList: true },
...
const handlePermissionUserDelete = async (user) => {
	...
	const index = rolePermissionUserSelect.value.findIndex(
...
const handleRoleUserDelete = async (user) => {
	// 通过唯一标识（如userId）查找用户在数组中的索引
	const index = tableShowRef.value.tableList.list.findIndex(
			(item) => item.userId === user.userId
	);
