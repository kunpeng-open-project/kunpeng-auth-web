	...
	<div class="main">
		...
		<KPTableTree ref="tableTreeRef" :event-bus="eventBus" :query-params="queryParams" :init-list="false" :list-api="basic.listApi" :table-column="tableColumn" checkbox :add-button="basic.addButtonAuth" :update-button="basic.updateButtonAuth" :del-button="basic.delButtonAuth" :details-button-row="basic.detailsButtonAuth" update-button-row del-button-row :table-key="basic.tableKey" :sort-api="basic.sortApi" :del-api="basic.delApi" @open-edit-dialog="openEditDialog" action-width="190px">
			...
			<template #menuType="{ row }">
				...
				<el-tag v-if="row.menuType === 'B'" type="warning">按钮</el-tag>
				<el-tag v-if="row.menuType === 'I'" type="danger">接口</el-tag>
			</template>
			
			<template #visible="{ row }">
				<el-tag v-if="row.visible === 1" type="success">显示</el-tag>
				<el-tag v-if="row.visible === 0" type="danger">不显示</el-tag>
			</template>
			
			<template #isEnable="{ row }">
				<el-switch v-model="row.isEnable" inline-prompt :active-value="1" active-text="正常" :inactive-value="0" inactive-text="停用" @click="handleSwitchStatus(row)"/>
			</template>
		</KPTableTree>
...
let tableColumn: TableColumn[] = [
	...
	{ prop: 'menuType', label: '菜单类型', sort: true },
	{ prop: 'isEnable', label: '是否启用', sort: true },
	{ prop: 'visible', label: '是否显示', sort: true },
