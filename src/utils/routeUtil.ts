...
export function routeUtil(title: string, name: string, path: string) {
    const route = useRoute();
    const router = useRouter();
    const parameter = isEmpty(route.params)? route.query : route.params;

    function toPage(model: "query" | "params", parameter: LocationQueryRaw | RouteParamsRaw) {
    ...
    const initToPage = (model: "query" | "params") => {
        if (parameter) toPage(model, parameter);
    };

    return { toPage, initToPage, parameter, router };
}
