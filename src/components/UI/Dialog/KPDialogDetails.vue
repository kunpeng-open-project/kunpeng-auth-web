        ...
        <el-descriptions :column="column" :border="border" class="t_detail" :style="{ '--label-width': labelWidth }">
          <el-descriptions-item label-align="right" :label-width="labelWidth" v-for="(item, index) in detailsColumn" :key="index" :label="item.label" :span="item.span ? item.span : 1">
            <!--
                        ...
                        -->
            <KPButton v-if="item.ability && item.ability.includes('formatJson')" :content="formattedFieldTitles[item.prop]" button-style="font-size: 18px;" type="success" :icon="formattedFieldTitles[item.prop] === '格式化' ? 'ri:code-box-fill' : 'ri:code-box-line'" link @click="handleFormatJson(item.prop)" />
            <KPButton v-if="item.ability && item.ability.includes('copy')" content="复制" button-style="font-size: 18px;" icon="ri:file-copy-2-line" link @click="handleCopy(detailsMessage[item.prop])" />
            ...
            <div v-else>
              <div v-if="String(detailsMessage[item.prop]).includes('\n')" v-html="item.render ? item.render[detailsMessage[item.prop]] : item.isList ? (detailsMessage[item.prop] && detailsMessage[item.prop].length > 0 ? detailsMessage[item.prop].join(item.isList) : '') + '&nbsp;&nbsp;' + (item.suffix ? item.suffix : '') : (detailsMessage[item.prop] ? String(detailsMessage[item.prop]).replaceAll('\n', '<br>') : '') + '&nbsp;&nbsp;' + (item.suffix ? item.suffix : '')"></div>
            </div>

            <div v-if="!String(detailsMessage[item.prop]).includes('\n')">
              {{ item.render ? item.render[detailsMessage[item.prop]] : item.isList ? (detailsMessage[item.prop] && detailsMessage[item.prop].length > 0 ? detailsMessage[item.prop].join(item.isList) : "") + "&nbsp;&nbsp;" + (item.suffix ? item.suffix : "") : (detailsMessage[item.prop] ? detailsMessage[item.prop] : "") + "&nbsp;&nbsp;" + (item.suffix ? item.suffix : "") }}
            </div>
...
// 接收父组件的值 变成普通数据
let { eventBus, detailsApi, tableKey, column } = props
...
eventBus.on("openDetailsDialog", async (row: any) => {
  ...
  const body = await queryTableDetails(props.apiPath, detailsApi, {
